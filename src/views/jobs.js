  ...
  return `
    <div class="container mb-2 animate-on-scroll">
      ...
      <div class="grid-cards">
        ${jobs.map((j, index) => `
          <a href="${j.link}" target="_blank" rel="noopener noreferrer" class="scroll-card animate-on-scroll" style="animation-delay: ${index * 0.1}s">
            <img src="${j.imageUrl}" class="scroll-card-img" alt="${j.title}" loading="lazy" />
            <div class="scroll-card-content">
              <div class="card-icon-header" style="margin-bottom: 10px;">
                <span class="card-badge"><i class="ph-fill ph-briefcase"></i> ${j.type}</span>
              </div>
              <h3>${j.title}</h3>
              <p>${j.desc}</p>
